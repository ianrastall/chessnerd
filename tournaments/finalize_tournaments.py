...
def polish_pgn_tags(input_path):
    """Runs pgn-extract to order tags strictly using roster.txt"""
    temp_polished = input_path + ".polished"
    cmd = [
        PGN_EXTRACT_PATH, input_path,
        "-R", "roster.txt", "--xroster", "-w", "9999",
        "-o", temp_polished
    ]
    try:
...
def main():
    ...
    if new_entries: bulk_update_html(new_entries)
    
    print("\nProcessing complete.")
    choice = input("Delete finalized files from _REVIEW? (y/n): ").lower()
    if choice == 'y':
